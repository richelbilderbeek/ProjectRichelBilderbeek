...
struct ChangingTransparentSprite : public QGraphicsPixmapItem
{
  explicit ChangingTransparentSprite(
      ...
      maxy(0)
  {
    ...
    const int width = i.width();
    const int height = i.height();
    const int dz = std::rand();
    for (int y=0;y!=height;++y)
    {
      for (int x=0;x!=width;++x)
      {
        QPoint pos(x,y);
        QColor color = i.pixel(pos);
        if (color == QColor(0,0,0)) continue;
        color.setRed((color.red() + dz) % 256);
        color.setGreen((color.green() + dz) % 256);
        color.setBlue((color.blue() + dz) % 256);
        i.setPixel(pos,color.rgb());
      }
    }
    this->setPixmap(this->pixmap().fromImage(i));
  ...
  void advance(int)
  {
    ...
    for (int y=0;y!=height;++y)
    {
      for (int x=0;x!=width;++x)
      {
        ...
        QColor color = i.pixel(pos);
        if (color == QColor(0,0,0)) continue;
        color.setRed((color.red() + 1) % 256);
        color.setGreen((color.green() + 1) % 256);
        color.setBlue((color.blue() + 1) % 256);
        i.setPixel(pos,color.rgb());
      }
    }
    this->setPixmap(this->pixmap().fromImage(i));
...
int main(int argc, char *argv[])
{
  ...
  std::vector<boost::shared_ptr<ChangingTransparentSprite> > sprites;
  //Add multiple sprites
  const int n_sprites = 3;
  for (int i=0; i!=n_sprites; ++i)
  {
    assert(QFile::exists(":/images/Bloem.png"));
    boost::shared_ptr<ChangingTransparentSprite> sprite(
      new ChangingTransparentSprite(":/images/Bloem.png"));
