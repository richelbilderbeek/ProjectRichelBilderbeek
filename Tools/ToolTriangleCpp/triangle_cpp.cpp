...
const int max_filename_size = 2048;

/* Maximum number of characters in a line read from a file (including the    */
/*   null).                                                                  */

const int max_inputline_size = 1024;
...
)
{
  ...
  for (i = start_index; i < argc; i++) {
    if (args[i][0] == '-') {
      for (j = start_index; args[i][j] != '\0'; j++) {
        if (args[i][j] == 'p') {
          b->poly = 1;
	}
#ifndef CDT_ONLY
	...
	if (args[i][j] == 'S') {
          ...
          while ((args[i][j + 1] >= '0') && (args[i][j + 1] <= '9')) {
            j++;
            b->steiner = b->steiner * 10 + (int) (args[i][j] - '0');
          }
        }
#endif /* not CDT_ONLY */
#ifndef REDUCED
      ...
      assert(i < argc); //RJCB
      strncpy(b->innodefilename, args[i].c_str(), max_filename_size - 1);
      b->innodefilename[max_filename_size - 1] = '\0';
    }
  }
  ...
  b->goodangle = cos(b->minangle * boost::math::constants::pi<double>() / 180.0);
  if (b->goodangle == 1.0) {
    b->offconstant = 0.0;
  } else {
...
/*                                                                           */
/*****************************************************************************/
Triangle *triangletraverse(Mesh * const m)
{
...
)
{
  ...
  errbound = iccerrboundC * permanent + resulterrbound * std::abs(det);
...
)
{
  ...
  permanent = (std::abs(bdxcdy) + std::abs(cdxbdy)) * alift
  ...
  const double permanent)
{
  ...
  errbound = o3derrboundC * permanent + resulterrbound * std::abs(det);
...
)
{
  ...
  permanent = (std::abs(bdxcdy) + std::abs(cdxbdy)) * std::abs(adheight)
...
void triangleinit(Mesh * const m)
{
  poolzero(&m->vertices);
...
char * readline(char * const string, FILE * const infile, char * /* infilename */)
{
  ...
  do {
    ...
    result = std::fgets(string, max_inputline_size, infile);
    //printf("Result: <begin>%s<end>\n",result); //RJCB
...
)
{
  ...
  /* Read the holes. */
  char * stringptr = readline(inputline, polyfile, polyfilename);
