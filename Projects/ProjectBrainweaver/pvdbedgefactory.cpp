...
const boost::shared_ptr<pvdb::Edge> pvdb::EdgeFactory::FromXml(
  const std::string& s,
  const std::vector<boost::shared_ptr<pvdb::Node> >& nodes)
{
  ...
  const boost::shared_ptr<pvdb::Edge> edge(new Edge(concept,x,y,nodes[from],tail_arrow,nodes[to],head_arrow));
  assert(edge);
  return edge;
}
    ...
    const boost::shared_ptr<pvdb::Node> to)
{
  ...
  for(const boost::shared_ptr<pvdb::Concept> concept: test_concepts)
  {
    {
      const boost::shared_ptr<pvdb::Edge> edge(new Edge(concept,1.2,3.4,from,false,to,true));
