...
void QtPvdbConceptMapEditWidget::AddEdge(const QtPvdbNodeItem * const from, const QtPvdbNodeItem* const to)
{
  assert(from);
  assert(to);
  assert(from != to);
  ...
  //Does this edge already exists? If yes, modify it
  {
    ...
    if (iter != edges.end())
    {
      ...
      if (qtedge->GetArrow()->GetToItem()   == to && !qtedge->GetArrow()->HasHead()) { qtedge->GetArrow()->SetHasHead(true); }
      if (qtedge->GetArrow()->GetFromItem() == to && !qtedge->GetArrow()->HasTail()) { qtedge->GetArrow()->SetHasTail(true); }
      this->scene()->update();
      return;
    }
  }
