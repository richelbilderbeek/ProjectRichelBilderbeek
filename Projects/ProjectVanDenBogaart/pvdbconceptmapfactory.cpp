...
{
  std::vector<boost::shared_ptr<pvdb::ConceptMap> > v(17);
  ...
  assert(std::count_if(v.begin(),v.end(),[](const boost::shared_ptr<pvdb::ConceptMap>& p) { return p; } ) == static_cast<int>(v.size()));
  //assert(std::all_of(v.begin(),v.end(),[](const boost::shared_ptr<pvdb::ConceptMap>& p) { return p; } ));
  return v;
}
