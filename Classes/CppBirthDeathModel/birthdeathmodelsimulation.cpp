...
double ribi::bdm::Simulation::CalcNext(const double x)
{
  const double r{
    CountExtant() * (m_parameters.BirthRate() + m_parameters.DeathRate())
  };
...
double ribi::bdm::Simulation::CalcNext(
  const double x,
...
) const
{
  ...
  m_t += dt;
  const auto dx = random_exponential * volatility;
  return x + (dt * dx);
}
...
void ribi::bdm::Simulation::Test() noexcept
{
  ...
  //Run a Brownian motion process
  {
    ...
    const ribi::bdm::Parameters parameters(volatility);
    ribi::bdm::Simulation sim(parameters);
  ...
  //Worked example
  {
    ...
    std::vector<double> random_exponentials(10);
    std::generate(begin(random_exponentials),end(random_exponentials),
      [&exponential_distribution,&rng]() { return exponential_distribution(rng); }
    );
    if (!"Show randoms")
    {
      std::copy(begin(random_exponentials),end(random_exponentials),
    ...
    for (int i=0; i!=10; ++i)
    {
      const double random_exponential{random_exponentials[i]};
      if (verbose) { std::cout << i << ": " << x << '\n'; }
      x = sim.CalcNext(x,random_exponential);
      xs.push_back(x);
    }
