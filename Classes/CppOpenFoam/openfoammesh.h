...
struct Mesh
{
  ...
  //static const std::vector<boost::shared_ptr<Cell> > CreateCells(
  //  const Files& files);
  //static const std::vector<boost::shared_ptr<Cell> > CreateCells(
  //  const Files& files, const std::vector<boost::shared_ptr<Face>>& faces);
  static const std::vector<boost::shared_ptr<Face> > CreateFaces(
    const Files& files,const std::vector<boost::shared_ptr<Point>>& points);
  static const std::vector<boost::shared_ptr<Point> > CreatePoints(const Files& files);
