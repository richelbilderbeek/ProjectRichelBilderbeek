...
if [ ! -d qwt-6.1 ]
then
  echo "Checkout Qwt 6.1 from SVN with HTTP"
  svn checkout http://svn.code.sf.net/p/qwt/code/branches/qwt-6.1 
else
  echo "Qwt 6.1 folder present\n"
fi

if [ ! -d qwt-6.1 ]
then
  echo "Checkout Qwt 6.1 from SVN with SVN"
...
echo "If Windows build failed fo the following:"
echo "Open qwtconfig.pri and change all 'QWT_CONFIG += [something]'s to"
echo "'QWT_CONFIG -= [something]'s, except 'QWT_CONFIG += QwtPlot'"
