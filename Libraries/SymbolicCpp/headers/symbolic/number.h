...
template <> Rational<Number<void> >::operator double() const
{
 ...
 std::cerr << "convert to double : "
      << pr.first.numerictype().name() << std::endl;
...
Numeric::match(const Numeric &n1,const Numeric &n2)
{
 ...
 if(t1 == type_int)
 {
  ...
  if(t2 == type_rational)
   return std::pair<Number<void>,Number<void> >
          ...
          ,n2);
  std::cerr << "Numeric cannot use " << t2.name() << std::endl;
 ...
 std::cerr << "Numeric cannot use " << t1.name() << std::endl;
 throw SymbolicError(SymbolicError::UnsupportedNumeric);
 return std::pair<Number<void>,Number<void> >(n1,n2);
}
...
PatternMatches
Numeric::match(const Symbolic &s, const std::list<Symbolic> &) const
{
 PatternMatches l;
...
template <class T> void Number<T>::print(std::ostream &o) const
{ o << n; }
